...
void NSFTrackDialog::InitList()
{
	...
	// resize the window in case the new song has a different number of tracks/channels
	CRect rect, rect2;
	GetClientRect(&rect);
	GetWindowRect(&rect2);
	...
	// adjust the window height based on how many tracks/channels there are, and apply the new size
	SetWindowPos(NULL, 0, 0, max_width, 246 + ((m_maxtrk - 5) * 17) + m_keydlg.m_keyheader.MaxHeight() + m_keydlg.SynthesiaHeight(), SWP_NOMOVE | SWP_NOZORDER);
...
BOOL NSFTrackDialog::OnInitDialog()
{
	...
	// set the total initial size of the window
	CRect rect, rect2;
	GetClientRect(&rect);
	GetWindowRect(&rect2);
	int min_height = rect2.Height() - rect.Height();
